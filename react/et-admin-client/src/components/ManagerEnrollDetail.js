...
const ManagerEnrollDetail = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://43.202.85.129:8081/manager/${concertManagerNo}`)
      .then((response) => {
  ...
  const handleAgree = () => {
    axios
      .post(`http://43.202.85.129:8081/manager/agree/${concertManagerNo}`)
      .then((response) => {
        alert('승인이 완료되었습니다.');
        window.history.back();
      })
  ...
  const handleDelete = () => {
    if (window.confirm('정말로 삭제하시겠습니까?')) {
      axios
        .post(`http://43.202.85.129:8081/manager/delete/${concertManagerNo}`)
        .then((response) => {
          alert('삭제가 완료되었습니다.');
          window.history.back();
        })
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(`http://43.202.85.129:8081/manager/update/${concertManagerNo}`, serverFormData)
      .then((response) => {
        alert(response.data);
      })
      .catch((error) => {
